...
result <- result %>% 
  mutate(p_value_mcse = sqrt(p_value_power*(1-p_value_power)/num_iter)) %>% 
  mutate(p_value_upper = p_value_power + qnorm(0.975)*p_value_mcse,
         p_value_lower = p_value_power - qnorm(0.975)*p_value_mcse) 

write_csv(result, glue("F:/yifei/UCL stats for clinical trial/Simulation with Rafael/Simulations/result/significance.csv"))
